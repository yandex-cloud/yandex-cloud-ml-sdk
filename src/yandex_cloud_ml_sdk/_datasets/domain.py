...
class BaseDatasets(BaseDomain, Generic[DatasetTypeT]):
    ...
    async def _list(
        ...
        *,
        ...
        if isinstance(status, str):
            status = DatasetStatus._from_str(status)

        request = ListDatasetsRequest(
            folder_id=self._folder_id,
            status=get_defined_value(status, 0),
...
class AsyncDatasets(BaseDatasets[AsyncDataset]):
...
class Datasets(BaseDatasets[Dataset]):
    _dataset_impl = Dataset

    __create = run_sync(BaseDatasets[Dataset]._create)
    __get = run_sync(BaseDatasets[Dataset]._get)
    __list = run_sync_generator(BaseDatasets[Dataset]._list)
    __list_upload_formats = run_sync(BaseDatasets[Dataset]._list_upload_formats)
