...
class AsyncSearchIndexUploader:
    ...
    async def _upload_single_file(self, file_metadata: FileMetadata, content: bytes) -> AsyncFile | None:
        ...
        try:
            ...
            uploaded_file = await self.sdk.files.upload_bytes(
                ...
                expiration_policy=self.config.file_expiration_policy,
    ...
    async def _create_search_index(self, files: list[File]) -> AsyncSearchIndex:
        ...
        operation = await self.sdk.search_indexes.create_deferred(
            ...
            description=self.config.index_description,
            labels=self.config.index_labels,
            ttl_days=self.config.index_ttl_days,
            expiration_policy=self.config.index_expiration_policy,
        )
